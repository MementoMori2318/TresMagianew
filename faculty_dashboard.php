...
// Query to get schedules linked to the faculty member
$scheduleDropdownQuery = "
    SELECT s.id, CONCAT(s.day_of_week, ' ', s.start_time, '-', s.end_time, ' ', su.subject_name, ' ', se.section_name) AS schedule_info FROM schedules s INNER JOIN subject su ON s.subject_id = su.subject_id INNER JOIN section se ON s.section_id = se.section_id INNER JOIN user_schedules us ON us.schedules_id = s.id WHERE us.users_id = ?

";
$dropdownStmt = $conn->prepare($scheduleDropdownQuery);
$dropdownStmt->bind_param("i", $_SESSION['id']);  // Pass the faculty ID from the session
$dropdownStmt->execute();
$dropdownResult = $dropdownStmt->get_result();

// Set the active schedule ID
$activeScheduleId = $selectedScheduleId ?: null;

// Attendance query for the selected schedule and date
...
$attendanceQuery = "
    ...
    FROM 
        user_schedules us
    JOIN 
        users u ON us.users_id = u.id
    LEFT JOIN 
        attendance a ON us.schedules_id = a.schedules_id 
        AND a.users_id = u.id
        AND a.date = ?
    WHERE 
        us.schedules_id = ?
...
$attendanceStmt->bind_param("si", $selectedDate, $activeScheduleId);
                            ...
                            <select id="scheduleSelect" name="schedule" class="form-select form-select-sm me-2" onchange="this.form.submit()" style="width: auto;">
                                ...
                                if ($dropdownResult->num_rows > 0) {
                                    while ($row = $dropdownResult->fetch_assoc()) {
                                        ...
                                        echo "<option value='{$row['id']}' $selected>{$row['schedule_info']}</option>";
                                    }
                                } else {
